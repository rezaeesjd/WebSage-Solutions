...
Tags: bokun, bookings, wp ecommerce, importer, shortcode, dashboard
...
== Description ==
Import Bokun to WP Ecommerce and Custom Fields (a.k.a. Bokun Bookings Management) lets tour and activity operators pull reservations from the Bokun API, persist them as the `bokun_booking` custom post type, and work with the data directly inside WordPress. The plugin includes booking dashboards, booking-history DataTables with CSV export, public-facing shortcodes, and utilities for syncing product-tag media.

== Features ==
* Save multiple Bokun API key/secret pairs from a repeatable UI that writes to the `bokun_api_credentials` option.
